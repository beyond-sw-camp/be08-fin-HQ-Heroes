...
<template>
  <div class="layout-topbar">
    <div class="layout-topbar-logo-container">
      ...
      <router-link to="/" class="layout-topbar-logo">
        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 537.64 563.26">
          <path id="uuid-fb6bba6d-324d-4625-a98c-3d990729dcd8-114-164-127-213"
            ...
            fill="#f8a8ab" />
          <rect x="254.14" y="514.38" width="20.94" height="29.71" transform="translate(529.23 1058.47) rotate(-180)"
            fill="#f8a8ab" />
          <path
            ...
            fill="#2f2e43" />
          <rect x="196.13" y="514.38" width="20.94" height="29.71" transform="translate(413.21 1058.47) rotate(-180)"
            fill="#f8a8ab" />
          <path
          ...
          <polygon
            ...
            fill="#6c63ff" />
          <polygon points="211.28 76.35 198.63 90.54 172.97 96.7 129.22 291.22 213.52 305.5 237 107.39 211.28 76.35"
            fill="#2f2e43" />
          <polygon
          ...
          <polygon
            ...
            fill="#2f2e43" />
          <path id="uuid-4155b336-10d9-4b14-826d-07551e167be9-115-165-128-214"
    ...
    <div class="layout-topbar-actions">
      ...
      <div v-else>
        ...
        <!-- PrimeVue 회원가입 버튼 -->
        <Button icon="pi pi-user-plus" label="회원가입" severity="success" outlined rounded @click="goToSignUp"
          class="ml-2" />
      </div>
    </div>
  </div>
  ...
  <Dialog v-model:visible="showReissueDialog" :modal="true" :closable="false"
    ...
    <div class="flex justify-center">
      <Button icon="pi pi-refresh" label="시간 갱신" @click="handleReissue"
        class="w-full py-2 px-4 font-bold rounded-lg text-white" />
    </div>
  </Dialog>
  ...
  <Drawer v-model:visible="notificationDrawerVisible" position="right" style="width: 40%">
    ...
    <DataTable v-if="!selectNotificationsList" :value="notifications" :rows="9" :paginator="true"
      responsiveLayout="scroll" selectionMode="single" @row-click="openNotificationModal" removableSort>
      <!-- 시간 컬럼 -->
      <Column style="width: 20%" header="시간" sortable>
      ...
      <!-- 상태 -->
      <Column field="notificationStatus" header="상태" sortable>
        <template #body="slotProps">
    ...
    <!-- 보낸 알림 -->
    <DataTable v-else :value="notifications" :rows="9" :paginator="true" responsiveLayout="scroll"
      selectionMode="single" @row-click="openNotificationModal" removableSort>
      <!-- 시간 컬럼 -->
      <Column field="createAt" header="시간" style="width: 18%" sortable>
        <template #body="slotProps">
          <span>{{ formatDate(slotProps.data.createdAt) }}</span>
        </template>
      </Column>
      ...
      <!-- 보낸 사람 컬럼 -->
      <Column field="receiverName" header="받은 사람" style="width: 18%" sortable>
        <template #body="slotProps">
      ...
      <!-- 카테고리 -->
      <Column field="categoryName" header="카테고리" style="width: 18%" sortable>
        <template #body="slotProps">
          <span>{{ slotProps.data.categoryName }}</span>
        </template>
      </Column>
    ...
    <template #footer>
      <div class="flex items-center gap-2">
        <Button label="Close" icon="pi pi-times" class="flex-auto" outlined
          @click="notificationDrawerVisible = false" />
      </div>
    </template>
  </Drawer>
</template>
