...
export default {
  ...
  methods: {
    ...
    filterSalaries() {
      this.filteredSalaries = this.salaries.filter(salary => salary.year === this.selectedYear);
      this.selectedSalaryIndex = null;
      this.selectedSalary = null; // Reset selected salary when filtering
    },
    selectSalary(index) {
      this.selectedSalaryIndex = index;
      this.selectedSalary = this.filteredSalaries[index];
    },
  },
};
</script>
