...
<template>
    <div class="card">
        <div class="font-semibold text-xl mb-4">월 근태 현황</div>
        <DataTable
        ...
        >
            ...
            <Column field="className" header="부 서" style="min-width: 12rem">
                <template #body="{ data }">
                    {{ data.className }}
                </template>
            </Column>
            <Column field="studentName" header="팀" style="min-width: 12rem">
                <template #body="{ data }">
                    {{ data.studentName }}
                </template>
            </Column>
            <Column field="address" header="이 름" style="min-width: 12rem">
                <template #body="{ data }">
                    {{ data.address }}
                </template>
            </Column>
            <Column field="openDt" header="날 짜" dataType="date" style="min-width: 10rem">
                <template #body="{ data }">
                    {{ formatDate(new Date(data.openDt)) }}
                </template>
            </Column>
            <Column field="closeDt" header="출근 시간" dataType="date" style="min-width: 10rem">
                <template #body="{ data }">
                    {{ formatDate(new Date(data.closeDt)) }}
                </template>
            </Column>
            <Column field="closeDt" header="퇴근 시간" dataType="date" style="min-width: 10rem">
                <template #body="{ data }">
                    {{ formatDate(new Date(data.closeDt)) }}
                </template>
            </Column>
            <Column field="closeDt" header="총 근무 시간" dataType="date" style="min-width: 10rem">
                <template #body="{ data }">
                    {{ formatDate(new Date(data.closeDt)) }}
                </template>
            </Column>
            <Column field="closeDt" header="상 태" dataType="date" style="min-width: 10rem">
                <template #body="{ data }">
                    {{ formatDate(new Date(data.closeDt)) }}
                </template>
            </Column>
