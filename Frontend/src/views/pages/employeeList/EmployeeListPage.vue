<template>
    <div class="card">
        ...
        >
                ...
                <div class="flex justify-between items-center">
                    <div class="flex items-center gap-2">
                        <Dropdown
                            ...
                            @change="filterByDepartmentAndTeam"
                            class="mr-2"
                        />
                        <Dropdown
            ...
            <Column field="department" header="부 서" style="min-width: 12rem">
                <template #body="{ data }">
                    {{ data.department }}
                </template>
            </Column>
            <Column field="teamName" header="팀" style="min-width: 12rem">
                <template #body="{ data }">
                    {{ data.teamName }}
                </template>
            </Column>
            <Column field="employeeName" header="이 름" style="min-width: 12rem">
                <template #body="{ data }">
                    {{ data.employeeName }}
                </template>
            </Column>
            <Column field="position" header="직 책" style="min-width: 12rem">
                <template #body="{ data }">
                    {{ data.position }}
                </template>
            </Column>
            <Column field="employeeId" header="사 번" style="min-width: 12rem">
                <template #body="{ data }">
                    {{ data.employeeId }}
                </template>
            </Column>
            <Column field="hireDate" header="입사일" dataType="date" style="min-width: 10rem">
                <template #body="{ data }">
...
async function fetchEmployeeList() {
    employees.value = [
        { employeeNo: 1, employeeName: '홍길동', department: '개발', teamName: '팀A', position: '개발자', employeeId: 'E001', hireDate: '2022-01-01', praiseCount: 0 },
        { employeeNo: 2, employeeName: '이순신', department: '개발', teamName: '팀B', position: '팀장', employeeId: 'E002', hireDate: '2021-02-01', praiseCount: 0 },
        { employeeNo: 3, employeeName: '김유신', department: '디자인', teamName: '팀C', position: '디자이너', employeeId: 'E003', hireDate: '2020-03-01', praiseCount: 0 }
    ];
...
function filterByDepartmentAndTeam() {
    filteredEmployees.value = employees.value.filter((employee) => {
        const matchesDepartment = selectedDepartment.value && selectedDepartment.value.name !== '전체'
            ? employee.department === selectedDepartment.value.name
            : true;
        const matchesTeam = selectedTeam.value && selectedTeam.value.name !== '전체'
            ? employee.teamName === selectedTeam.value.name
            : true;
